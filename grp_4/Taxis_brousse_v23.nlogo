...
clients-choisis  ; Nombre de villageois ayant la même destination que le taxi et a embarquer a l'instant t+1
...
to creer-villageois ; Creer un nombre de villageois grace a un slider répartis aléatoirement dans l'espace
...
Le modèle concerne la production de services de transports de voyageurs non régulé en milieu rural africain, services consistant à acheminemer des villadeois vers un ou des marchés.
Le modèle doit permettre d'étudier les phénomènes de convergence sur un ou des points centraux (les marchés) à partir de plusieurs points secondaires (les stations) situés sur un réseau de transport et vers lesquels convergent une population dispersée dans l’espace. Cette double convergence alimente un processus d’auto-renforcement des centres, lui-même produisant une différenciation de l’espace.
Il s'agit d'observer plusieurs processus : l’auto renforcement (polarisation sur quelques stations et différenciation des marchés); l’émergence de bassins d’attraction; la différenciation croissante des axes de circulation; une efficacité différenciée des comportements des taxis influencés par les phénomènes de polarisation.


PROCESSUS DU MODELE
-----------

- Les villageois, au début dispersés dans l’espace, convergent vers les villages-stations selon un modèle gravitaire. Chaque villageois souhaite prendre un taxi pour se rendre à un marché vers lequel il est attiré selon un même modèle gravitaire.
- Les taxis parcourent le réseau à la recherche de clients de manière aléatoire. 
Arrivés à une station, ils prennent des passagers allant dans une même direction (tirage au sort a priori favorable à la destination pour laquelle les passagers sont les plus nombreux). 
- Une fois les passagers embarqués, les taxis se dirigent vers le marché en suivant le plus court chemin. En route, ils peuvent embarquer d’autres passagers pour la même destination, dans la limite de leur capacité de charge
- Arrivés au marché, les passagers sont débarqués, ils « sortent » du jeu, et les taxis repartent à la recherche de clients.
- L’attractivité des villages-stations est fonction du nombre de voyageurs qui y ont été embarqués. Plus le nombre de villageois ayant pris un taxis en un point est élevé, plus ce point est attractif pour les villageois.
- L’attractivité des marchés est fonction du nombre de voyageurs qui y sont débarqués. Plus le nombre de villageois arrivés à un  marché est élevé, plus il est attractif pour les villageois.
- De nouveaux villageois arrivent par « vagues » successives dès que le nombre de villageois encore « en jeu » diminue fortement, ce qui contribue à faire vivre le modèle en permanence.


UTILISATION
-----------

- Configurer un réseau de manière aléatoire en définissant le nombre de nœuds et de marchés, nombre de liens créés (les nœuds sont liés les uns aux autres selon une fonction de distance).

- Une fois le réseau configuré, faire varier les paramètres : nombre de villageois, nombre de taxis, leur capacité de chargement, différentiel de vitesse entre taxis et villageois (piétons), nombre de ticks par jour.

Les indicateurs observés sont :
- La polarisation des marchés qui est fonction du nombre de villageois arrivés.
- L’efficacité des taxis qui est le rapport entre le nombre de nœuds empruntés et le nombre de nœuds sur lesquels le taxi a embarqué des passagers. Quand ce rapport tend vers la valeur 1, l’efficacité est maximale (le taxi embarque des passagers dans tous - les nœuds parcourus), quand il tend vers 0, elle est minimale.
- Le nombre maximum de passages sur un lien (les liens deviennent de plus en plus gros en fonction du nombre de passages)
- Le taux de desserte qui est un indicateur d’efficacité globale : il s’agit du rapport entre le nombre de villageois arrivés à destination et le nombre total de villageois pour un « jour » (soit à chaque fois qu’une nouvelle population de villageois est « lancée » dans le jeu)


OBSERVATIONS
---------

- Sur le processus d’auto-renforcement : après quelques itérations, apparition d'une structure centrale dans le réseaux représentés par les axes et quelques nœuds qui permettent d’accéder aux marchés. Selon la configuration initiale, on voit émerger soit une sorte de colonne vertébrale, soit deux ou plusieurs bassins (en fonction du nombre de marchés choisi au départ) assez peu reliés entre eux. Les figures qui se dessinent se rapprochent plus de celle de l’arbre (un tronc et des branches) que de celle de l’étoile. 

- Apparition de zones enclavées où les villageois n’ont que peu ou pas de chance d’accéder à un taxi. Progressivement, les villageois se tournent vers d’autres villages stations, plus éloignés mais plus attractifs car la probabilité d’y avoir un taxi y est plus grande. L’indice d’efficacité globale, le taux de desserte, évolue par phases. Dans un premier temps il reste bas, puis atteint progressivement des valeurs assez élevées au fur et à mesure que le nombre de villageois encore en jeu se réduit, forçant les taxis à parcourir les branches les plus éloignées et isolées du graphes. L’arrivée de nouveaux villageois fait chuter la valeur de l’indice qui finit par se stabiliser autour de valeurs qui montrent globalement une efficacité assez faible. La réorientation progressive des villageois (piétons) vers les villages les plus attractifs a-t-elle un impact sur cet indicateur ? À voir en laissant le jeu tourner assez longtemps.


DYSFONCTIONNEMENT A RESOUDRE
-----------

- A l'initialisation, les villageois qui vont directement au marché à pieds ne meurent pas, du coup les taxis les chargent lorsqu'ils sont au marché et ils les déchargent quand ils y reviennent.

- Certains taxis ne prennent aucun villageois à des stations alors qu'ils sont vides.

- Certains taxis peuvent rester sur un même lien pendant un certain temps.


PERSPECTIVES
---------

- Les transporteurs adaptent-ils leurs stratégies entre logique de concurrence et logique de complémentarité ?
- Quels sont les impacts de ces stratégies sur l’émergence de formes spatiales (bassins / réseaux) ? 
- Quel est le rôle des configurations initiales dans l’émergence de phénomènes de polarisation ?
- Comment pourraient évoluer les demandes et les comportements des villageois pour s’adapter à l’offre ?
- Quels pourraient être les leviers de régulation ? : nbre de taxis et lignes de transport fixes...
